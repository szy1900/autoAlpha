...
class Alphas(object):
    def __init__(self, df_data):
        df_data['returns'] = df_data['close']/df_data['close'].shift(1)-1
        df_data['delta'] = df_data['close']-df_data['close'].shift(1)
        ...
        self.delta = df_data['delta']

        # self.vwap = (df_data['amount'] )/(df_data['volume'] )
    ...
    def alpha006(self):
        df = -1 * correlation(self.open, self.volume, 5) ##背离原理
        return df.replace([-np.inf, np.inf], 0).fillna(value=0)

    # Alpha#7	 ((adv20 < volume) ? ((-1 * ts_rank(abs(delta(close, 7)), 60)) * sign(delta(close, 7))) : (-1* 1))
    def alpha007(self):
        adv20 = sma(self.volume, 20)
        alpha = -1 * ts_rank(abs(delta(self.close, 7)), 60) * sign(delta(self.close, 7)) ##价格弹性指标
        alpha[adv20 >= self.volume] = -1#量要大
        return alpha
    ...
    def alpha024(self):
        cond = delta(sma(self.close, 100), 100) / delay(self.close, 100) <= 0.03
    ...
    def alpha030(self):
        close_temp = self.close.copy()
        delta_close = delta(close_temp, 1)
        inner = sign(delta_close) + sign(delay(delta_close, 1)) + sign(delay(delta_close, 2))
        return ((1.0 - rank(pd.DataFrame(inner))['close']) * ts_sum(self.volume, 5)) / ts_sum(self.volume, 20)
