...
import itertools
import os
from PIL import Image as PSGImg
from PIL import ImageTk as PSGImgTK
import io
...
# ====================================================================== #
# Other handy functions that don't belong anywhere else                  #
# ====================================================================== #
def append_images(images, direction='horizontal',
                  background_color=None, aligment='center', output_type='PIL'):
    """
    Appends a list of images in an horizontal/vertical direction. Default returns a PIL image. If
    you give a single image you can convert it to diferent formats.

    :param images: List of PIL images or base64 image strings
    :type images: Union[str,list]
    :param direction: Direction of concatenation, 'horizontal' or 'vertical'
    :type direction: (str)
    :param background_color: Background color, takes a tuple representing RGB value (default: transparent)
    :type background_color: (tuple)
    :param aligment: Alignment mode if images need padding;
                    'left', 'right', 'top', 'bottom', or 'center'
    :param output_type: The type of image data to return. Accepts 'PIL', PhotoImage', 'Base64' and 'All'
    :type output_type: (str)
    :return: Returns a new concatened image, can be new PIL image object, a PhotoImage object to be displayed
             in Tkinter or a base64 string. Can also return a dictionary with all formats.
    """
    if images is not None:
        if type(images) is list or type(images) is tuple:
            # Opens images depending if they are normal files or base64 image strings
            decoded_images = []
            for image in images:
                if type(image) is bytes:  # image is base64 string
                    # Decodes base64 image strings into readable file format
                    decoded_images += [PSGImg.open(io.BytesIO(base64.b64decode(image)))]
                else:  # image is a file path
                    decoded_images += [PSGImg.open(image)]

            # Gets width and height of all images
            ...
            new_im = PSGImg.new('RGBA', (new_width, new_height), color=background_color)
            ...
            for im in decoded_images:
                if direction == 'horizontal':
                    ...
                    offset += im.size[1]
        else:  # Single image defined, just convert it
            if type(images) is bytes:  # image is base64 string
                # Decodes base64 image strings into readable file format
                new_im = PSGImg.open(io.BytesIO(base64.b64decode(images)))
            else:  # image is a file path
                new_im = PSGImg.open(images)

        if output_type == 'PIL':
            return new_im  # returns PIL image object
        elif output_type == 'PhotoImage':
            try:
                tk_photoimage_image = PSGImgTK.PhotoImage(new_im)
            except:
                # If you call this as a standalone function and you haven't initialized Tkinter this will save from a crash
                print('append_images() ERROR: Tkinter is not yet initialized so will not include this format')
                tk_photoimage_image = None
            return tk_photoimage_image  # returns TK PhotoImage object
        elif output_type == 'Base64':
            buffered = io.BytesIO()
            new_im.save(buffered, format="PNG")
            base64_image = base64.b64encode(buffered.getvalue())
            return base64_image  # return base64 image string
            ...
            new_im.save(buffered, format="PNG")
            base64_image = base64.b64encode(buffered.getvalue())
            return {'PIL': new_im, 'PhotoImage': tk_photoimage_image, 'Base64': base64_image}
    else:
...
class Tree(Element):
    ...
    def __init__(self, data=None, headings=None, visible_column_map=None, col_widths=None, col0_width=10,
                 ...
                 background_color=None, header_text_color=None, header_background_color=None, header_font=None,
                 num_rows=None, row_height=None, pad=None, key=None, tooltip=None,
                 right_click_menu=None, visible=True, metadata=None):
        ...
        self.checked_icon = b''
        self.unchecked_icon = b'iVBORw0KGgoAAAANSUhEUgAAAA0AAAALCAYAAACksgdhAAAABmJLR0QA/wD/AP+gvaeTAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAB3RJTUUH5AQWFwAtnUW0NwAAADNJREFUKM9jbGpq+s9ABKitrWVgZGRkZGBgYGCBCZACmBjIAKOa6K6J8f////+JVgxNEQC/kwmO77tl1gAAAABJRU5ErkJggg=='
    ...
    def _checkbox_manager(self, node):
        """
        Not a user function. This function manages the custom checkbox states in tree element

        :param node: Gets TreeData type node
        :type node: (TreeData)
        :return: PhotoImage object with appended icon and checkbox in one image
        """
        ...
        if type(node) is dict:
            ...
            checkbox_enabled = node['checkbox_enabled']
        else:
            key = node.key
            icon = node.icon
            checked = node.checked
        ...
        if key not in self.CheckboxState:
            self.CheckboxState[key] = {}

        tmp_checkbox_icon = None
        if checked is not None:
            if checked:
                tmp_checkbox_icon = self.checked_icon
            else:
                tmp_checkbox_icon = self.unchecked_icon
            self.CheckboxState[key].update({'checked': checked})
        else:
            if self.CheckboxState[key]['checked'] is not None:
                if self.CheckboxState[key]['checked']:
                    tmp_checkbox_icon = self.checked_icon
                else:
                    tmp_checkbox_icon = self.unchecked_icon

        if checkbox_enabled is not None:
            if checkbox_enabled:
                self.CheckboxState[key].update({'checkbox_enabled': True})
            else:
                self.CheckboxState[key].update({'checked': None})
                self.CheckboxState[key].update({'checkbox_enabled': False})
                tmp_checkbox_icon = None
        else:
            if self.CheckboxState[key]['checkbox_enabled'] is not None:
                if self.CheckboxState[key]['checkbox_enabled'] is False:
                    self.CheckboxState[key].update({'checked': None})
                    tmp_checkbox_icon = None

        no_icon = False
        tmp_images_to_append = None
        if icon is not None:
            if type(icon) is list or type(icon) is tuple:
                tmp_images_to_append = list(icon)
                self.CheckboxState[key].update({'icon': list(icon)})
                if tmp_checkbox_icon is not None:
                    tmp_images_to_append.append(tmp_checkbox_icon)
            else:
                if tmp_checkbox_icon is not None:
                    tmp_images_to_append = (icon, tmp_checkbox_icon)
                else:
                    tmp_images_to_append = icon
                self.CheckboxState[key].update({'icon': icon})
        else:
            if tmp_checkbox_icon is not None:
                if 'icon' in self.CheckboxState[key] and self.CheckboxState[key]['icon'] is not None:
                    tmp_icon = self.CheckboxState[key]['icon']
                    if tmp_checkbox_icon is not None:
                        if type(tmp_icon) is list or type(tmp_icon) is tuple:
                            tmp_images_to_append = list(tmp_icon)
                            tmp_images_to_append.append(tmp_checkbox_icon)
                        else:
                            tmp_images_to_append = (self.CheckboxState[key]['icon'], tmp_checkbox_icon)
                    else:
                        tmp_images_to_append = tmp_checkbox_icon
                else:
                    self.CheckboxState[key].update({'icon': None})
            else:
                no_icon = True
        ...
        if no_icon:
            ...
            photo = append_images(tmp_images_to_append, output_type='PhotoImage')
        return photo

    def change_checkbox_state(self, key=None, checked=None):
        """
            Checks or unchecks a tree node checkbox automatically, check state also can be changed manually
            providing checked value

        :param key: Node key
        :type key: (str)
        :param checked: To assign manual checked state, False = uncheked, True = checked
        :type checked: (bool)
        :return:
        """
        if self.CheckboxState[key]['checkbox_enabled']:
            if checked is not None:
                self.update(key=key, checked=checked)
            else:
                self.update(key=key, checked=not self.CheckboxState[key]['checked'])
    ...
    def add_treeview_data(self, node):
        ...
        if node.key != '':
            try:
                photo = self._checkbox_manager(node)
    ...
    def Update(self, values=None, key=None, value=None, text=None, icon=None, visible=None, checkbox_enabled=None, checked=None):
        """
        ...
        :param visible: control visibility of element
        :type visible: (bool)
        :param checkbox_enabled: True if the checkbox is visible in the tree node
        :type checkbox_enabled: bool
        ...
        if id:
            ...
            if checkbox_enabled is not None or icon is not None or checked is not None:
                node = {'key': key,
                        'icon': icon,
                        'checked': checked,
...
class TreeData(object):
    ...
    class Node(object):
        ...
        def __init__(self, parent, key, text, values, icon=None, checkbox_enabled=False, checked=False):
            """
            ...
            :param checkbox_enabled: True to display the checkbox on the node
            :type checkbox_enabled: bool
            :param checked: True if the node is checked
            :type checked: (bool)
            """
            ...
            self.checkbox_enabled = checkbox_enabled  # type: bool
    ...
    def Insert(self, parent, key, text, values, icon=None, checkbox_enabled=False, checked=False):
        """
        ...
        node = self.Node(parent, key, text, values, icon, checkbox_enabled, checked)
        self.tree_dict[key] = node
    ...
    for row_num, flex_row in enumerate(form.Rows):
        ...
        for col_num, element in enumerate(flex_row):
            ...
            # -------------------------  Tree placement element  ------------------------- #
                ...
                element.add_treeview_data(element.TreeData.root_node)
